...
RecoveryStatus = Literal["recovered", "no_legal_option", "needs_override", "failed"]
...
class Offer(BaseModel):
    ...
    def layovers(self, iata: dict[str, str]) -> list[Layover]:
        """Compute the connecting airports between consecutive segments.

        `iata` maps airport IATA -> ISO-2 country. Slice 1 passes a small
        demo map; Slice 2+ passes the bundled iata_country.csv loader.
        """
        out: list[Layover] = []
