...
*Done:* on one sandbox route: `offer verify` → `booking confirm-price` → `order create` → `order pay` → `order status` asserted `TICKETED`, plus `booking seat list`/`seat select`; `order pay` single-use and never retried; branching on `code` only.
