...
> "Corporate travel tools enforce policy; FLOAT is hired to beat the market — a portfolio manager with a mandate, a P&L it must defend, and losses it has to explain."
...
- Real sandbox booking rail: `offer verify` → `booking confirm-price` → `order create` → `order pay` → `order status` asserted `TICKETED`.
