...
FLOAT refits the existing repo; nothing is rebuilt beside it. The existing 3-screen IA (setup → live agent → outcome) maps 1:1 to **mandate → desk → close**.
...
6. **Write path** — `offer verify` → `booking confirm-price` → `order create` → `order pay` (single-use, **never retried**) → poll `order status` until **TICKETED** (**GUARD: assert the real outcome, not 200 OK**).
7. **Settle** — ledger entries: trade; reconcile (on `PRICE_CHANGED`: absorb-from-contingency vs re-quote — judgment call, never a second order); alloc (realized savings fund `booking seat select`).
...
| `alloc` | position_id, amount, seat ref | savings → `booking seat select` |
...
| `offer verify --offer-id` | freshness re-read before every write | read-only; at most one identical retry |
| `booking confirm-price` | price confirmation → booking-id | read-only; at most one identical retry |
| `order create --booking-id --passengers-stdin` | order creation | **write; NEVER retried.** On `ORDER_CREATION_UNKNOWN` / `DUPLICATE_BOOKING_SUSPECTED` → query `order status` only |
| `order pay --confirmation-id` | settlement | **write; single-use; NEVER retried under any circumstance** |
| `order status --order-no` | outcome assertion (poll until `TICKETED`) | read-only; `TICKETING_PENDING` is continuing, not failure |
| `booking seat list` / `booking seat select` | savings allocation | list read-only; select = write, never retried |
